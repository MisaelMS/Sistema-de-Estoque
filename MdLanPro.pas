{-------------------------------------------------------------------------------
 Nome da Unit: MdLanPro

 Empresa:      Consystec
 Sistema:      Sistema Contabil
 Versão:       0.1 Beta
 Autor:        Pedro Paulo Alexandrino
 Descrição:    Modelo de lançamentos
 Data Início:  01/01/2008
-------------------------------------------------------------------------------}
...
uses
  ...
  Buttons, DBGrids, cxLookAndFeelPainters, cxButtons, System.Actions,
  cxLookAndFeels, System.ImageList, cxImageList;

type
  TFMdLanPro = class(TForm)
...
function TFMdLanPro.VerifDup(Codig, Campo, Tabela: String): Boolean;
var
  qSQL : TOracleQuery;
  VlCodi : Integer;
...
procedure TFMdLanPro.AdicFiltro(const Campo: String; const Inteiro: Integer;
...
procedure TFMdLanPro.AdicFiltro(const Campo: String; const Valor: Double;
  const ZeroVazio: Boolean);
var
  DS : char;
begin
...
procedure TFMdLanPro.AdicFiltro(const Campo: String; Str: String);
begin
  if Str <> '' then begin
    Str := Trim(Str);
...
procedure TFMdLanPro.AdicFiltro(const Campo: String; const Dat: TDateTime;
  Data: Boolean);
begin
  if (Dat <> 0) then
...
procedure TFMdLanPro.AdicFiltro(const Condicao: String);
begin
  if Condicao <> '' then
    Filtros.Add(Condicao);
end;

procedure TFMdLanPro.AdicFiltro(const Campo: String; const D1, D2: TDateTime);
var
  S : string;
begin
...
procedure TFMdLanPro.AplicarFiltro;
var
  Str : string;
  K : Integer;
begin
...
procedure TFMdLanPro.LanUsarFiltr(const Value: String);
begin
  FLanFiltr := Value;
  if Value <> '' then
...
procedure TFMdLanPro.LanUsarOrdem(const Value: String);
begin
  FLanOrdem := Value;
...
procedure TFMdLanPro.TentaApagar(Dataset: TOracleDataset;
...
procedure TFMdLanPro.FormCreate(Sender: TObject);
begin
  Cascade;
...
procedure TFMdLanPro.FormDestroy(Sender: TObject);
begin
  FreeAndNil(Filtros);
  FreeAndNil(FLanStSQL);
end;

procedure TFMdLanPro.ProxControle(Direcao: Integer; var Tecla: Word);
begin
...
procedure TFMdLanPro.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if (Screen.ActiveControl is TcxDBCalcEdit) then
    if TcxDBCalcEdit(Screen.ActiveControl).DroppedDown then
      Exit;
...
procedure TFMdLanPro.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
const
  Dir_Prox = 0;
  Dir_Ante = 1;
...
procedure TFMdLanPro.AtualizaAcoes(Editando: Boolean);
var
  TemRegistro: Boolean;
begin
...
procedure TFMdLanPro.OraDsLancamAfterScroll(DataSet: TDataSet);
begin
  //actExcl.Enabled := not(DataSet.IsEmpty or (DataSet.State in dsEditModes));
end;

procedure TFMdLanPro.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFMdLanPro.ActLanSairExecute(Sender: TObject);
begin
  if ActiveControl.ClassType <> TDBGrid then
    Close;
end;

procedure TFMdLanPro.GanhaFocu(Sender: TObject);
begin
  ComponenteEnter(Sender);
end;

procedure TFMdLanPro.PerdeFocu(Sender: TObject);
begin
  ComponenteExit(Sender, TComponent(Sender).Tag);
end;

procedure TFMdLanPro.actAdicExecute(Sender: TObject);
begin                     
  OraDsLancam.Insert;
end;

procedure TFMdLanPro.actEditExecute(Sender: TObject);
begin
  OraDsLancam.Edit;
end;

procedure TFMdLanPro.actExclExecute(Sender: TObject);
begin
...
procedure TFMdLanPro.actCancExecute(Sender: TObject);
begin
  OraDsLancam.Cancel;
  OraDsLancam.CancelUpdates;
end;

procedure TFMdLanPro.actAtuaExecute(Sender: TObject);
begin
  OraDsLancam.Refresh;
end;

procedure TFMdLanPro.actGravExecute(Sender: TObject);
begin
  {}
end;

procedure TFMdLanPro.actConsExecute(Sender: TObject);
begin
  {}
end;

procedure TFMdLanPro.actFiltExecute(Sender: TObject);
begin
  {}
end;

procedure TFMdLanPro.actImprExecute(Sender: TObject);
begin
  {}
end;

procedure TFMdLanPro.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if actCanc.Enabled then
    Case MessageBox(GetActiveWindow, PChar(MSG_SALVCANC),
      PChar(Application.Title), MB_ICONQUESTION + MB_YESNOCANCEL) of
...
procedure TFMdLanPro.FormShow(Sender: TObject);
begin
  {tbnAdic.Caption := '';
  tbnEdit.Caption := '';
  tbnExcl.Caption := '';
